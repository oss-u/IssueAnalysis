...
> {
  render() {
    ...
    this.props.summaries.forEach((s) => {
      const uniqueAuthors = [...new Set(s.comments.map((comment) => comment.author.uname))]
      if (this.props.viewing && this.props.viewing === s.id) {
        summaryContent.push(
          <div className="Box flex-column m-1 p- color-border-info"
            ...
            <div className="lh-condensed text-semibold f6 m-1">
            Autogenerated summary of {s.comments.length} comment(s) by <span className="text-bold f6">{uniqueAuthors.join(", ")}</span>
            </div>
  ...
  loadSummaryComponent = () => {
    if (this.state.subsummaries.length) {
      ...
      if (summaries.length > 0) {
        return <SummaryComponent summaries={summaries}
